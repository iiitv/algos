...
public class DoubleEndedQueue
{
    ...
    public void enque_front(int arr[],int value,int n)// to insert from front
    {
        if(isfull(n)==1)
        {
            System.out.println("sorry the queue is full");return;
        }
       if(front==0)
     {  System.out.println("cannot enque from front");return;}
        else
        {
            if(back==-1)back++;
           if(front==-1)
           {front++;arr[front]=value;}
           else
           arr[--front]=value;
           return;
        }
     ...
     public void enque_back(int arr[],int value,int n)//to insert from back
    {
        ...
        if(back==n-1)
        {
             System.out.println("sorry cannot enque from back ");return;
        }
        if(isempty()==1)
        {
            front++;back++;
            arr[back]=value;return;
        }

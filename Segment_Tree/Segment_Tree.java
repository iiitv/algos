...
void update(int left,int right,int tree[],int arr[],int index,int ind,int value)//to update a given value at a particular index in O(logn)
{
...
int ans(int left,int right,int initial,int last,int tree[],int arr[],int index)// to provide the minimum value within a given range in O(logn)
{
    ...
    return min( ans(left,right,initial,mid,tree,arr,2*index+1) , ans(left,right,mid+1,last,tree,arr,2*index+2) ) ;
}
public static void main(String args[])
{
    ...
    while((tc--)!=0)
    {
       ...
       sg. build(0,n-1,tree,arr,0);//to construct segment tree in O(n)
        ...
        int l=1,r=5;
        System.out.println("the min value in the range 1 to 5 inclusive is:\n" +sg.ans(l-1,r-1,0,n-1,tree,arr,0));

       int ind=3,value=-20;
       sg.update(0,n-1,tree,arr,0,ind-1,value);//to update the value at index ind in array,thereby updating whole tree
